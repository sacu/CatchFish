...
namespace Sacu.Factory
{
    public class SABaseFactory
    {
        ...
        virtual public void KillFactory()
        {//放假工厂因子
            int length = graphWorkers.Count;
            int i = 0;
            while (graphWorkers.Count > 0)
            {
                destroyFactory(graphWorkers[i]);
            }
            ...
            {
                data = dataWorkers[i];
                removeDataWorker(data.getName());
                dataWorkers[i] = null;
            }

            eventDispatcher.Clear();//事件域
            workersTreeList.Clear();//工人名册
            repeatObjects.Clear();//已存在图形
            graphWorkers.Clear();//视图worker控制列表
            dataWorkers.Clear();//数据worker控制列表
        }
        /**
        ...
        private void changeWorkerState(bool state)
        {
            ...
            if (_start) workersTreeList[GRAPH_WORKER].callAllOnRegister();
