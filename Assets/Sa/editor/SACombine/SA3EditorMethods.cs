...
public class SA3EditorMethods : SA2EditorMethodsInterface
{
    private List<Texture2D> _texturesWithReadWriteFlagSet = new List<Texture2D>();
    private Dictionary<Texture2D, SATextureFormatInfo> _textureFormatMap = new Dictionary<Texture2D, SATextureFormatInfo>();
    ...
    void SetTextureFormat(Texture2D tx, SATextureFormatInfo tfi, bool addToList, bool setNormalMap)
    {
        ...
        if (ai != null && ai is UnityEditor.TextureImporter)
        {
            ...
            Debug.Log(s);
            TextureImporter textureImporter = (TextureImporter)ai;
            SATextureFormatInfo restoreTfi = new SATextureFormatInfo(textureImporter.textureFormat,
                                                                tfi.platform,
    ...
    {
        //			if (LOG_LEVEL >= SA2LogLevel.debug) SA2Log.Log(SA2LogLevel.debug,"Assigning atlas " + texturePath + " to result material " + target + " for property " + texPropName,LOG_LEVEL);
    ...
    public bool CheckPrefabTypes(SAObjsToCombineTypes objToCombineType, List<GameObject> objsToMesh)
    {
        for (int i = 0; i < objsToMesh.Count; i++)
        {
                ...
                pt == UnityEditor.PrefabType.DisconnectedModelPrefabInstance)
            {
                ...
                if (objToCombineType == SAObjsToCombineTypes.prefabOnly)
                {
            ...
            else if (objToCombineType == SAObjsToCombineTypes.sceneObjOnly)
            {
                //these are prefabs
