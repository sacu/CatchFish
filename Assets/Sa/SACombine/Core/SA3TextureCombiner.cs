...
[Serializable]
public class SA3TextureCombiner {
    ...
    public SA2TextureCombineResults textureBakeResults
    {
    ...
    [SerializeField]
    protected SA2PackingAlgorithmEnum _packingAlgorithm = SA2PackingAlgorithmEnum.POT;// UnitysPackTextures;
    public SA2PackingAlgorithmEnum packingAlgorithm
    {
    ...
    class SATexSet
    {
        public SACombineMaterialTexture[] ts;
        public List<Material> mats;
        public List<GameObject> gos;
        public int idealWidth; //all textures will be resized to this size
        public int idealHeight;

        public SATexSet(SACombineMaterialTexture[] tss)
        {
            ts = tss;
    ...
    *  <param name="textureEditorMethods">If called from the editor should be an instance of SA2EditorMethods. If called at runtime should be null.</param>
    *  <remarks>Combines meshes and generates texture atlases</remarks> */
    public bool CombineTexturesIntoAtlases(ProgressUpdateDelegate progressInfo, SAAtlasesAndRects resultAtlasesAndRects, Material resultMaterial, List<GameObject> objsToMesh, List<Material> allowedMaterialsFilter, SA2EditorMethodsInterface textureEditorMethods = null)
    {
    ...
    bool _CollectPropertyNames(Material resultMaterial, List<string> texPropertyNames)
    {
        ...
        for (int i = 0; i < shaderTexPropertyNames.Length; i++)
        {
            if (m.HasProperty(shaderTexPropertyNames[i]))
            {
                ...
                {
                    if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("Result material has non-zero offset. This is may be incorrect.");
                }
                if (m.GetTextureScale(shaderTexPropertyNames[i]) != new Vector2(1f, 1f))
                {
                    if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("Result material should may be have tiling of 1,1");
                }
            }
        }
        ...
        for (int i = 0; i < _customShaderPropNames.Count; i++)
        {
            if (m.HasProperty(_customShaderPropNames[i]))
            {
                ...
                {
                    if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("Result material has non-zero offset. This is probably incorrect.");
                }
                if (m.GetTextureScale(_customShaderPropNames[i]) != new Vector2(1f, 1f))
                {
                    if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("Result material should probably have tiling of 1,1.");
                }
            }
            else
            {
                if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("Result material shader does not use property " + _customShaderPropNames[i] + " in the list of custom shader property names");
            }
        }
    ...
    bool _CombineTexturesIntoAtlases(ProgressUpdateDelegate progressInfo, SAAtlasesAndRects resultAtlasesAndRects, Material resultMaterial, List<GameObject> objsToMesh, List<Material> allowedMaterialsFilter, SA2EditorMethodsInterface textureEditorMethods)
    {
        bool success = false;
        try
        {
    ...
    bool __CombineTexturesIntoAtlases(ProgressUpdateDelegate progressInfo, SAAtlasesAndRects resultAtlasesAndRects, Material resultMaterial, List<string> texPropertyNames, List<GameObject> objsToMesh, List<Material> allowedMaterialsFilter, SA2EditorMethodsInterface textureEditorMethods)
    {
        if (LOG_LEVEL >= SA2LogLevel.debug) Debug.Log("__CombineTexturesIntoAtlases atlases:" + texPropertyNames.Count + " objsToMesh:" + objsToMesh.Count + " _fixOutOfBoundsUVs:" + _fixOutOfBoundsUVs);
        ...
        */
        List<SATexSet> distinctMaterialTextures = new List<SATexSet>(); //one per distinct set of textures
    ...
    bool __Step1_CollectDistinctMatTexturesAndUsedObjects(List<GameObject> allObjsToMesh,
                                                         ...
                                                         SA2EditorMethodsInterface textureEditorMethods,
                                                         List<SATexSet> distinctMaterialTextures, //Will be populated
                                                         List<GameObject> usedObjsToMesh) //Will be populated, is a subset of allObjsToMesh
    {
        ...
        for (int i = 0; i < allObjsToMesh.Count; i++)
        {
            ...
            if (LOG_LEVEL >= SA2LogLevel.debug) Debug.Log("Collecting textures for object " + obj);
            ...
            Mesh sharedMesh = SAUtility.GetMesh(obj);
            if (sharedMesh == null)
            {
            ...
            Material[] sharedMaterials = SAUtility.GetGOMaterials(obj);
            ...
            for (int matIdx = 0; matIdx < sharedMaterials.Length; matIdx++)
            {
                ...
                bool mcOutOfBoundsUVs = SAUtility.hasOutOfBoundsUVs(sharedMesh, ref uvBounds, matIdx);
                ...
                {
                    if (!SAUtility.validateOBuvsMultiMaterial(sharedMaterials))
                    {
                        if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("Object " + obj.name + " uses the same material on multiple submeshes. This may generate strange resultAtlasesAndRects especially when used with fix out of bounds uvs. Try duplicating the material.");
                    }
                }

                //collect textures scale and offset for each texture in objects material
                SACombineMaterialTexture[] mts = new SACombineMaterialTexture[texPropertyNames.Count];
                for (int j = 0; j < texPropertyNames.Count; j++)
                {
                        ...
                        if (txx != null)
                        {
                            if (txx is Texture2D)
                            {
                                ...
                                else
                                {
                                    ...
                                    //SA2Log.Log(SA2LogLevel.warn,obj.name + " in the list of objects to mesh uses Texture "+tx.name+" uses format " + f + " that is not in: ARGB32, RGBA32, BGRA32, RGB24, Alpha8 or DXT. These formats cannot be resized. Combine will create duplicates.");
                    ...
                    if (tx == null)
                    {
                        if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("No texture selected for " + texPropertyNames[j] + " in object " + allObjsToMesh[i].name + ". A 2x2 clear texture will be generated and used in the atlas.");
                    }
                    if (mcOutOfBoundsUVs)
                    {
                    ...
                    mts[j] = new SACombineMaterialTexture(tx, offset, scale, obUVoffset, obUVscale);
                }

                //Add to distinct set of textures if not already there
                SATexSet setOfTexs = new SATexSet(mts);
                SATexSet setOfTexs2 = distinctMaterialTextures.Find(x => x.IsEqual(setOfTexs, _fixOutOfBoundsUVs));
                if (setOfTexs2 != null)
                {
    ...
    int __Step2_CalculateIdealSizesForTexturesInAtlasAndPadding(List<SATexSet> distinctMaterialTextures)
    {
        int _padding = _atlasPadding;
        if (distinctMaterialTextures.Count == 1)
        {
            if (LOG_LEVEL >= SA2LogLevel.info) Debug.Log("All objects use the same textures in this set of atlases. Original textures will be reused instead of creating atlases.");
            _padding = 0;
        }
        else
        {
            for (int i = 0; i < distinctMaterialTextures.Count; i++)
            {
                if (LOG_LEVEL >= SA2LogLevel.debug) Debug.Log("Calculating ideal sizes for texSet TexSet " + i + " of " + distinctMaterialTextures.Count);
                SATexSet txs = distinctMaterialTextures[i];
                txs.idealWidth = 1;
                txs.idealHeight = 1;
                int tWidth = 1;
                int tHeight = 1;
                ...
                {
                    SACombineMaterialTexture matTex = txs.ts[j];
                    if (!matTex.scale.Equals(Vector2.one) && distinctMaterialTextures.Count > 1)
                    {
                        if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("Texture " + matTex.t + "is tiled by " + matTex.scale + " tiling will be baked into a texture with maxSize:" + _maxTilingBakeSize);
                    }
                    if (!matTex.obUVscale.Equals(Vector2.one) && distinctMaterialTextures.Count > 1 && _fixOutOfBoundsUVs)
                    {
                        if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("Texture " + matTex.t + "has out of bounds UVs that effectively tile by " + matTex.obUVscale + " tiling will be baked into a texture with maxSize:" + _maxTilingBakeSize);
                    }
                    if (matTex.t != null)
                    {
                        ...
                        {
                            if (LOG_LEVEL >= SA2LogLevel.trace) Debug.Log("    matTex " + matTex.t + " " + dim + " has a bigger size than " + tWidth + " " + tHeight);
                            tWidth = (int)dim.x;
                            tHeight = (int)dim.y;
                        }
                    }
                ...
                if (LOG_LEVEL >= SA2LogLevel.trace) Debug.Log("    Ideal size is " + tWidth + " " + tHeight);
    ...
    void __Step3_BuildAndSaveAtlasesAndStoreResults(ProgressUpdateDelegate progressInfo, List<SATexSet> distinctMaterialTextures, List<string> texPropertyNames, int _padding, SA2EditorMethodsInterface textureEditorMethods, SAAtlasesAndRects resultAtlasesAndRects, Material resultMaterial)
    {
        ...
        Rect[] uvRects;
        if (_packingAlgorithm == SA2PackingAlgorithmEnum.POT)
        {
        ...
        _destroyTemporaryTextures();
        if (textureEditorMethods != null) textureEditorMethods.SetReadFlags(progressInfo);
        if (report != null && LOG_LEVEL >= SA2LogLevel.info) Debug.Log(report.ToString());
    }

    Rect[] __CreateAtlasesMBTexturePacker(ProgressUpdateDelegate progressInfo, int numAtlases, List<SATexSet> distinctMaterialTextures, List<string> texPropertyNames, Material resultMaterial, Texture2D[] atlases, SA2EditorMethodsInterface textureEditorMethods, int _padding)
    {
        Rect[] uvRects;
        if (distinctMaterialTextures.Count == 1)
        {
            if (LOG_LEVEL >= SA2LogLevel.debug) Debug.Log("Only one image per atlas. Will re-use original texture");
            uvRects = new Rect[1];
            uvRects[0] = new Rect(0f, 0f, 1f, 1f);
            for (int i = 0; i < numAtlases; i++)
            {
                SACombineMaterialTexture dmt = distinctMaterialTextures[0].ts[i];
                atlases[i] = dmt.t;
        ...
        else
        {
            ...
            SA2TexturePacker tp = new SA2TexturePacker();
            tp.doPowerOfTwoTextures = _combineTexturePackerForcePowerOfTwo;
            int atlasSizeX = 1;
            int atlasSizeY = 1;
            ...
            if (LOG_LEVEL >= SA2LogLevel.info) Debug.Log("Generated atlas will be " + atlasSizeX + "x" + atlasSizeY + " (Max atlas size for platform: " + atlasMaxDimension + ")");
            for (int i = 0; i < numAtlases; i++)
            {
                GC.Collect();
                ...
                {
                    if (LOG_LEVEL >= SA2LogLevel.trace) SA2Log.Trace("Adding texture {0} to atlas {1}", distinctMaterialTextures[j].ts[i].t == null ? "null" : distinctMaterialTextures[j].ts[i].t.ToString(), texPropertyNames[i]);
                    Rect r = uvRects[j];
                    ...
                    List<Material> ms = distinctMaterialTextures[j].mats;
                    for (int k = 0; k < ms.Count; ++k)
                    {
                        ms[k].SetTextureScale(texPropertyNames[i], new Vector2(r.width, r.height));
                        ms[k].SetTextureOffset(texPropertyNames[i], new Vector2(r.x, r.y));
                ...
                atlas.Apply();
                if (LOG_LEVEL >= SA2LogLevel.debug) Debug.Log("Saving atlas " + texPropertyNames[i] + " w=" + atlas.width + " h=" + atlas.height);
                atlases[i] = atlas;
                ...
                //setting texture   直接替换资源
                //string prefabPth = UnityEditor.AssetDatabase.GetAssetPath(resultMaterial);
                ...
                //string texturePath = folderPath + baseName + "-" + texPropertyNames[i] + "-atlas" + i + ".png";
                //for (int j = 0; j < distinctMaterialTextures.Count; j++)
                //{
                //    List<Material> ms = distinctMaterialTextures[j].mats;
                //    for (int k = 0; k < ms.Count; ++k)
                //    {
                //        ms[k].SetTexture(texPropertyNames[i], (Texture2D)(UnityEditor.AssetDatabase.LoadAssetAtPath(texturePath, typeof(Texture2D))));
                //        Debug.Log("texturePath is : " + texturePath);
                //    }
                //}
    ...
    Rect[] __CreateAtlasesUnityTexturePacker(ProgressUpdateDelegate progressInfo, int numAtlases, List<SATexSet> distinctMaterialTextures, List<string> texPropertyNames, Material resultMaterial, Texture2D[] atlases, SA2EditorMethodsInterface textureEditorMethods, int _padding)
    {
        Rect[] uvRects;
        if (distinctMaterialTextures.Count == 1)
        {
        ...
        else
        {
            ...
            { //i is an atlas "MainTex", "BumpMap" etc...
                if (LOG_LEVEL >= SA2LogLevel.debug) Debug.LogWarning("Beginning loop " + i + " num temporary textures " + _temporaryTextures.Count);
                for (int j = 0; j < distinctMaterialTextures.Count; j++)
                { //j is a distinct set of textures one for each of "MainTex", "BumpMap" etc...
                    SATexSet txs = distinctMaterialTextures[j];
                    ...
                    if (tx.width != tWidth || tx.height != tHeight)
                    {
                        ...
                        if (LOG_LEVEL >= SA2LogLevel.debug) Debug.LogWarning("Copying and resizing texture " + texPropertyNames[i] + " from " + tx.width + "x" + tx.height + " to " + tWidth + "x" + tHeight);
                ...
                {
                    if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("The maximum possible atlas size is 4096. Textures may be shrunk");
                }
                ...
                if (i == 0)
                {
                    ...
                    if (LOG_LEVEL >= SA2LogLevel.info) Debug.Log("Estimated atlas minimum size:" + Math.Sqrt(estArea).ToString("F0"));
                    ...
                    if (LOG_LEVEL >= SA2LogLevel.info) Debug.Log("After pack textures atlas size " + atlases[i].width + " " + atlases[i].height);
    ...
    Texture2D _copyTexturesIntoAtlas(Texture2D[] texToPack, int padding, Rect[] rs, int w, int h)
    {
        ...
        SAUtility.setSolidColor(ta, Color.clear);
        for (int i = 0; i < rs.Length; i++)
        {
            ...
            {
                t = SAUtility.resampleTexture(t, ww, hh);
    ...
    Vector2 GetAdjustedForScaleAndOffset2Dimensions(SACombineMaterialTexture source)
    {
        ...
        if (LOG_LEVEL >= SA2LogLevel.debug) Debug.LogWarning("GetAdjustedForScaleAndOffset2Dimensions: " + source.t + " " + source.obUVoffset + " " + source.obUVscale);
    ...
    public Texture2D GetAdjustedForScaleAndOffset2(SACombineMaterialTexture source)
    {
        if (source.offset.x == 0f && source.offset.y == 0f && source.scale.x == 1f && source.scale.y == 1f)
        {
            if (_fixOutOfBoundsUVs)
        ...
        if (LOG_LEVEL >= SA2LogLevel.debug) Debug.LogWarning("GetAdjustedForScaleAndOffset2: " + source.t + " " + source.obUVoffset + " " + source.obUVscale);
    ...
    public void CopyScaledAndTiledToAtlas(SACombineMaterialTexture source, int targX, int targY, int targW, int targH, bool _fixOutOfBoundsUVs, int maxSize, Color[][] atlasPixels, int atlasWidth, bool isNormalMap, ProgressUpdateDelegate progressInfo = null)
    {
        if (LOG_LEVEL >= SA2LogLevel.debug) Debug.Log("CopyScaledAndTiledToAtlas: " + source.t + " inAtlasX=" + targX + " inAtlasY=" + targY + " inAtlasW=" + targW + " inAtlasH=" + targH);
        ...
        if (t == null)
        {
            ...
            if (isNormalMap)
            {
                SAUtility.setSolidColor(t, new Color(.5f, .5f, 1f));
            }
            else
            {
                SAUtility.setSolidColor(t, Color.clear);
            }
        }
        t = _addWatermark(t);
        ...
        for (int i = 0; i < w; i++)
        {
            ...
            for (int j = 0; j < h; j++)
            {
                ...
                atlasPixels[targY + j][targX + i].a = 10;
            }
        }
    ...
    {
        Texture2D tx = SAUtility.createTextureCopy(t);
    ...
    {
        Texture2D tx = SAUtility.resampleTexture(t, w, h);
        _temporaryTextures.Add(tx);
        return tx;
    }
    ...
    {
        if (LOG_LEVEL >= SA2LogLevel.debug) Debug.Log("Destroying " + _temporaryTextures.Count + " temporary textures");
        for (int i = 0; i < _temporaryTextures.Count; i++)
        {
            SAUtility.Destroy(_temporaryTextures[i]);
        }
        _temporaryTextures.Clear();
    }
    ...
    public void SuggestTreatment(List<GameObject> objsToMesh, Material[] resultMaterials, List<string> _customShaderPropNames)
    {
        ...
        for (int i = 0; i < objsToMesh.Count; i++)
        {
            ...
            Mesh m = SAUtility.GetMesh(obj);
            Rect r = new Rect();
            for (int j = 0; j < ms.Length; j++)
            {
                if (SAUtility.hasOutOfBoundsUVs(m, ref r, j))
                {
