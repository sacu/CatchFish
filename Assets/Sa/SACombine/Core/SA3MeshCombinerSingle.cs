...
public class SA3MeshCombinerSingle : SA3MeshCombiner
{
    [System.Serializable]
    class SADynamicGameObject : IComparable<SADynamicGameObject>
    {
        ...
        */
        ...
        public int CompareTo(SADynamicGameObject b)
        {
    ...
    public override SA2TextureCombineResults textureBakeResults
    {
        set
        {
            ...
            {
                if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("If material bake result is changed then objects currently in combined mesh may be invalid.");
            }
            _textureBakeResults = value;
        }
    }

    public override SARenderType renderType
    {
        set
        {
            if (value == SARenderType.skinnedMeshRenderer && _renderType == SARenderType.meshRenderer)
            {
    ...
    public override GameObject resultSceneObject
    {
        set
        {
            if (_resultSceneObject != value)
            {
                ...
                if (_mesh != null && _LOG_LEVEL >= SA2LogLevel.warn)
                {
    ...
    [SerializeField]
    List<SADynamicGameObject> mbDynamicObjectsInCombinedMesh = new List<SADynamicGameObject>();
    Dictionary<int, SADynamicGameObject> _instance2combined_map = new Dictionary<int, SADynamicGameObject>();
    ...
    SADynamicGameObject instance2Combined_MapGet(int gameObjectID)
    {
        return _instance2combined_map[gameObjectID];
    }

    void instance2Combined_MapAdd(int gameObjectID, SADynamicGameObject dgo)
    {
    ...
    bool instance2Combined_MapTryGetValue(int gameObjectID, out SADynamicGameObject dgo)
    {
    ...
    {
        if (lightmapOption == SA2LightmapOptions.generate_new_UV2_layout || lightmapOption == SA2LightmapOptions.preserve_current_lightmapping)
        {
    ...
    void _initialize()
    {
        ...
        if (_mesh == null)
        {
            if (LOG_LEVEL >= SA2LogLevel.debug) SA2Log.LogDebug("_initialize Creating new Mesh");
            _mesh = GetMesh();
        }
    ...
    bool _collectMaterialTriangles(Mesh m, SADynamicGameObject dgo, Material[] sharedMaterials, OrderedDictionary sourceMats2submeshIdx_map)
    {
        ...
        for (int i = 0; i < numTriMeshes; i++)
        {
            ...
            if (LOG_LEVEL >= SA2LogLevel.debug) SA2Log.LogDebug("Collecting triangles for: " + dgo.name + " submesh:" + i + " maps to submesh:" + dgo.targetSubmeshIdxs[i] + " added:" + dgo._submeshTris[i].Length, LOG_LEVEL);
        }
        return true;
    }

    bool _collectOutOfBoundsUVRects2(Mesh m, SADynamicGameObject dgo, Material[] sharedMaterials, OrderedDictionary sourceMats2submeshIdx_map)
    {
        ...
        {
            Rect r = new Rect();
            SAUtility.hasOutOfBoundsUVs(m, ref r, i);
            dgo.obUVRects[i] = r;
        }
        return true;
    }
    ...
    bool _showHide(GameObject[] goToShow, GameObject[] goToHide)
    {
        ...
        for (int i = 0; i < goToHide.Length; i++)
        {
            ...
            {
                if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("Trying to hide an object " + goToHide[i] + " that is not in combined mesh");
                return false;
            }
        }
        ...
        for (int i = 0; i < goToShow.Length; i++)
        {
            ...
            {
                if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("Trying to show an object " + goToShow[i] + " that is not in combined mesh");
                return false;
            }
        }
    ...
    bool _addToCombined(GameObject[] goToAdd, int[] goToDelete, bool disableRendererInSource)
    {
        ...
        if (outputOption != SA2OutputOptions.bakeMeshAssetsInPlace &&
            renderType == SARenderType.skinnedMeshRenderer)
        {
        ...
        {
            Debug.LogWarning("There were vertices in the combined mesh but nothing in the Combine buffers. If you are trying to bake in the editor and modify at runtime, make sure 'Clear Buffers After Bake' is unchecked.");
        }

        int numResultMats = 1;
        if (textureBakeResults.doMultiMaterial) numResultMats = textureBakeResults.resultMaterials.Length;

        if (LOG_LEVEL >= SA2LogLevel.debug) SA2Log.LogDebug("==== Calling _addToCombined objs adding:" + _goToAdd.Length + " objs deleting:" + _goToDelete.Length + " fixOutOfBounds:" + textureBakeResults.fixOutOfBoundsUVs + " doMultiMaterial:" + textureBakeResults.doMultiMaterial + " disableRenderersInSource:" + disableRendererInSource, LOG_LEVEL);
        ...
        if (textureBakeResults.doMultiMaterial)
        {
            ...
            {
                SAMultiMaterial mm = textureBakeResults.resultMaterials[i];
        ...
        for (int i = 0; i < _goToDelete.Length; i++)
        {
            ...
            if (instance2Combined_MapTryGetValue(_goToDelete[i], out dgo))
            {
                ...
                if (renderType == SARenderType.skinnedMeshRenderer)
                {
            ...
            {
                if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("Trying to delete an object that is not in combined mesh");
            }
        }

        //now add
        List<SADynamicGameObject> toAddDGOs = new List<SADynamicGameObject>();
        int totalAddVerts = 0;
        int totalAddBones = 0;
        ...
        for (int i = 0; i < _goToAdd.Length; i++)
        {
            ...
            {
                SADynamicGameObject dgo = new SADynamicGameObject();

                GameObject go = _goToAdd[i];

                Material[] sharedMaterials = SAUtility.GetGOMaterials(go);
                ...
                else if (SAVersion.IsRunningAndMeshNotReadWriteable(m))
                {
                ...
                if (_goToAdd[i] != null)
                {
                    ...
                    Renderer r = SAUtility.GetRenderer(go);
                    ...
                    if (lightmapOption == SA2LightmapOptions.preserve_current_lightmapping)
                    {
                        if (lightmapIndex != r.lightmapIndex)
                        {
                            if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("Object " + go.name + " has a different lightmap index. Lightmapping will not work.");
                        }
                        if (!SAVersion.GetActive(go))
                        {
                            if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("Object " + go.name + " is inactive. Can only get lightmap index of active objects.");
                        }
                        if (r.lightmapIndex == -1)
                        {
                            if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("Object " + go.name + " does not have an index to a lightmap.");
                        }
                    }
                    dgo.lightmapIndex = r.lightmapIndex;
                    dgo.lightmapTilingOffset = SAVersion.GetLightmapTilingOffset(r);
                    ...
                    if (renderType == SARenderType.skinnedMeshRenderer) totalAddBones += dgo.numBones;
            ...
            {
                if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("Object " + _goToAdd[i].name + " has already been added");
                _goToAdd[i] = null;
            }
        }
        ...
        for (int i = 0; i < _goToAdd.Length; i++)
        {
            ...
            {
                SAUtility.DisableRendererInSource(_goToAdd[i]);
                if (LOG_LEVEL == SA2LogLevel.trace) Debug.Log("Disabling renderer on " + _goToAdd[i].name + " id=" + _goToAdd[i].GetInstanceID());
            }
        }
        ...
        if (LOG_LEVEL >= SA2LogLevel.debug) SA2Log.LogDebug("Verts adding:" + totalAddVerts + " deleting:" + totalDeleteVerts + " submeshes:" + newSubmeshTrisSize.Length + " bones:" + newBonesSize, LOG_LEVEL);
        ...
        for (int i = 0; i < newSubmeshTrisSize.Length; i++)
        {
            ...
            if (LOG_LEVEL >= SA2LogLevel.debug) SA2Log.LogDebug("    submesh :" + i + " already contains:" + submeshTris[i].Length + " tris to be Added:" + totalAddSubmeshTris[i] + " tris to be Deleted:" + totalDeleteSubmeshTris[i]);
        }

        if (newVertSize > 65534)
        {
            Debug.LogError("Cannot add objects. Resulting mesh will have more than 64k vertices. Try using a Multi-Combine component. This will split the combined mesh into several meshes. You don't have to re-configure the SA2TextureCombine. Just remove the SA2MeshCombine component and add a SA2MultiCombine component.");
            return false;
        }
        ...
        if (lightmapOption == SA2LightmapOptions.copy_UV2_unchanged ||
            lightmapOption == SA2LightmapOptions.preserve_current_lightmapping) nuv2s = new Vector2[newVertSize];
        ...
        {
            SADynamicGameObject dgo = mbDynamicObjectsInCombinedMesh[i];
            if (!dgo._beingDeleted)
            {
                if (LOG_LEVEL >= SA2LogLevel.debug) SA2Log.LogDebug("Copying obj in combined arrays idx:" + i, LOG_LEVEL);
                ...
                for (int subIdx = 0; subIdx < numResultMats; subIdx++)
                {
                    ...
                    if (LOG_LEVEL >= SA2LogLevel.debug) SA2Log.LogDebug("    Adjusting submesh triangles submesh:" + subIdx + " startIdx:" + sTriIdx + " num:" + sNumTris, LOG_LEVEL);
            ...
            {
                if (LOG_LEVEL >= SA2LogLevel.debug) SA2Log.LogDebug("Not copying obj: " + i, LOG_LEVEL);
        ...
        if (renderType == SARenderType.skinnedMeshRenderer) boneWeights = nboneWeights;
        ...
        {
            SADynamicGameObject dgo = toAddDGOs[i];
            GameObject go = _goToAdd[i];
            int vertsIdx = targVidx;
            //				Profile.StartProfile("TestNewNorm");
            Mesh mesh = SAUtility.GetMesh(go);
            ...
            {
                if (LOG_LEVEL >= SA2LogLevel.debug) SA2Log.LogDebug("Mesh " + dgo.name + " has more submeshes than materials");
                numTriSets = dgo.uvRects.Length;
            }
            else if (dgo.uvRects.Length > numTriSets)
            {
                if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("Mesh " + dgo.name + " has fewer submeshes than materials");
            }
            ...
            if (_doUV1)
            {
                nuv1s = SAVersion.GetMeshUV1s(mesh, LOG_LEVEL);
            ...
            if (LOG_LEVEL >= SA2LogLevel.debug) SA2Log.LogDebug("Added to combined:" + dgo.name + " verts:" + nverts.Length + " bindPoses:" + nbindPoses.Length, LOG_LEVEL);
        }
        if (LOG_LEVEL >= SA2LogLevel.debug) SA2Log.LogDebug("===== _addToCombined completed. Verts in buffer: " + verts.Length, LOG_LEVEL);
        return true;
    }

    void _copyAndAdjustUVsFromMesh(SADynamicGameObject dgo, Mesh mesh, int vertsIdx)
    {
        ...
        if (textureBakeResults.fixOutOfBoundsUVs == false)
        {
            ...
            if (allAreIdent)
            {
                ...
                if (LOG_LEVEL >= SA2LogLevel.debug) Debug.Log("All atlases have only one texture in atlas UVs will be copied without adjusting");
            }
        }
        if (needToModfyUVs)
        {
            ...
            {
                if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning(dgo.name + "has submeshes which share verticies. Adjusted uvs may not map correctly in combined atlas.");
            }
        }
        nuvs.CopyTo(uvs, vertsIdx);
        if (LOG_LEVEL >= SA2LogLevel.trace) Debug.Log("_copyAndAdjustUVsFromMesh copied " + nuvs.Length);
    }

    void _copyAndAdjustUV2FromMesh(SADynamicGameObject dgo, Mesh mesh, int vertsIdx)
    {
        ...
        { //has a lightmap
            ...
            if (LOG_LEVEL >= SA2LogLevel.trace) Debug.Log("_copyAndAdjustUV2FromMesh copied and modify for preserve current lightmapping " + nuv2s.Length);
        }
        else if (LOG_LEVEL >= SA2LogLevel.trace)
        {
    ...
    int _copyBonesBindPosesAndBoneWeightsFromMesh(GameObject go, SADynamicGameObject dgo, int vertsIdx, int bonesIdx)
    {
        Renderer r = SAUtility.GetRenderer(go);
        ...
        if (LOG_LEVEL >= SA2LogLevel.trace) Debug.Log("_copyBonesBindPosesAndBoneWeightsFromMesh copied " + nboneWeights.Length);
    ...
    Color[] _getMeshColors(Mesh m)
    {
        ...
        if (cs.Length == 0)
        {
            if (LOG_LEVEL >= SA2LogLevel.debug) SA2Log.LogDebug("Mesh " + m + " has no colors. Generating");
            if (_doCol) if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("Mesh " + m + " didn't have colors. Generating an array of white colors");
    ...
    Vector3[] _getMeshNormals(Mesh m)
    {
        ...
        if (ns.Length == 0)
        {
            if (LOG_LEVEL >= SA2LogLevel.debug) SA2Log.LogDebug("Mesh " + m + " has no normals. Generating");
            if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("Mesh " + m + " didn't have normals. Generating normals.");
            Mesh tempMesh = (Mesh)GameObject.Instantiate(m);
            tempMesh.RecalculateNormals();
            ns = tempMesh.normals;
            SAUtility.Destroy(tempMesh);
        }
        return ns;
    }
    ...
    Vector4[] _getMeshTangents(Mesh m)
    {
        ...
        if (ts.Length == 0)
        {
            if (LOG_LEVEL >= SA2LogLevel.debug) SA2Log.LogDebug("Mesh " + m + " has no tangents. Generating");
            if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("Mesh " + m + " didn't have tangents. Generating tangents.");
    ...
    Vector2[] _getMeshUVs(Mesh m)
    {
        ...
        if (uv.Length == 0)
        {
            ...
            if (LOG_LEVEL >= SA2LogLevel.debug) SA2Log.LogDebug("Mesh " + m + " has no uvs. Generating");
            if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("Mesh " + m + " didn't have uvs. Generating uvs.");
    ...
    Vector2[] _getMeshUV2s(Mesh m)
    {
        ...
        if (uv.Length == 0)
        {
            if (LOG_LEVEL >= SA2LogLevel.debug) SA2Log.LogDebug("Mesh " + m + " has no uv2s. Generating");
            if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("Mesh " + m + " didn't have uv2s. Lightmapping option was set to " + lightmapOption + " Generating uv2s.");
            uv = new Vector2[m.vertexCount];
            for (int i = 0; i < uv.Length; i++) { uv[i] = _HALF_UV; }
        }
        return uv;
    ...
    public override void UpdateSkinnedMeshApproximateBoundsFromBones()
    {
        if (outputOption == SA2OutputOptions.bakeMeshAssetsInPlace)
        {
            if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("Can't UpdateSkinnedMeshApproximateBounds when output type is bakeMeshAssetsInPlace");
            return;
        }
        if (bones.Length == 0)
        {
            if (verts.Length > 0) if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("No bones in SkinnedMeshRenderer. Could not UpdateSkinnedMeshApproximateBounds.");
            return;
        }
        if (_targetRenderer == null)
        {
            if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("Target Renderer is not set. No point in calling UpdateSkinnedMeshApproximateBounds.");
            return;
        }
        if (!_targetRenderer.GetType().Equals(typeof(SkinnedMeshRenderer)))
        {
            if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("Target Renderer is not a SkinnedMeshRenderer. No point in calling UpdateSkinnedMeshApproximateBounds.");
            return;
        }
    ...
    public override void UpdateSkinnedMeshApproximateBoundsFromBounds()
    {
        if (outputOption == SA2OutputOptions.bakeMeshAssetsInPlace)
        {
            if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("Can't UpdateSkinnedMeshApproximateBoundsFromBounds when output type is bakeMeshAssetsInPlace");
            return;
        }
        if (verts.Length == 0 || objectsInCombinedMesh.Count == 0)
        {
            if (verts.Length > 0) if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("Nothing in SkinnedMeshRenderer. Could not UpdateSkinnedMeshApproximateBoundsFromBounds.");
            return;
        }
        if (_targetRenderer == null)
        {
            if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("Target Renderer is not set. No point in calling UpdateSkinnedMeshApproximateBoundsFromBounds.");
            return;
        }
        if (!_targetRenderer.GetType().Equals(typeof(SkinnedMeshRenderer)))
        {
            if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("Target Renderer is not a SkinnedMeshRenderer. No point in calling UpdateSkinnedMeshApproximateBoundsFromBounds.");
            return;
        }
    ...
    {
        bool doBones = false;
        if (renderType == SARenderType.skinnedMeshRenderer) doBones = true;
        Apply(true, true, _doNorm, _doTan, _doUV, _doCol, _doUV1, doUV2(), doBones, uv2GenerationMethod);
    }

    public virtual void ApplyShowHide()
    {
        if (_validationLevel >= SA2ValidationLevel.quick && !ValidateTargRendererAndMeshAndResultSceneObj()) return;
        if (_mesh != null)
        {
            SAVersion.MeshClear(_mesh, true);
            ...
            if (LOG_LEVEL >= SA2LogLevel.trace) Debug.Log("ApplyShowHide");
        }
        else
        {
            Debug.LogError("Need to add objects to this combine before calling ApplyShowHide");
        }
    }
                      ...
                      GenerateUV2Delegate uv2GenerationMethod = null)
    {
        ...
        if (_mesh != null)
        {
            ...
            if (triangles || _mesh.vertexCount != verts.Length)
            {
                ...
                {
                    SAVersion.MeshClear(_mesh, true); //clear just triangles 
                }
                else
                {//clear all the data and start with a blank mesh
                    SAVersion.MeshClear(_mesh, false);
                }
            }
            ...
            if (normals)
            {
                ...
                else { Debug.LogError("normal flag was set in Apply but Combine didn't generate normals"); }
            }

            if (tangents)
            {
                if (_doTan) { _mesh.tangents = this.tangents; }
                else { Debug.LogError("tangent flag was set in Apply but Combine didn't generate tangents"); }
            }
            if (uvs)
            {
                if (_doUV) { _mesh.uv = this.uvs; }
                else { Debug.LogError("uv flag was set in Apply but Combine didn't generate uvs"); }
            }
            if (colors)
            {
                if (_doCol) { _mesh.colors = this.colors; }
                else { Debug.LogError("color flag was set in Apply but Combine didn't generate colors"); }
            }
            if (uv1)
            {
                if (_doUV1)
                {
                    SAVersion.MeshAssignUV1(_mesh, this.uv1s);
                }
                else { Debug.LogError("uv1 flag was set in Apply but Combine didn't generate uv1s"); }
            }
            if (uv2)
            {
            ...
            if (renderType != SARenderType.skinnedMeshRenderer && lightmapOption == SA2LightmapOptions.generate_new_UV2_layout)
            {
                if (uv2GenerationMethod != null)
                {
                    //Unwrapping.GenerateSecondaryUVSet(_mesh);
                    uv2GenerationMethod(_mesh);
                    if (LOG_LEVEL >= SA2LogLevel.trace) Debug.Log("generating new UV2 layout for the combined mesh ");
                }
                else
                {
            ...
            else if (renderType == SARenderType.skinnedMeshRenderer && lightmapOption == SA2LightmapOptions.generate_new_UV2_layout)
            {
                if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("UV2 cannot be generated for SkinnedMeshRenderer objects.");
            }
            if (renderType != SARenderType.skinnedMeshRenderer && lightmapOption == SA2LightmapOptions.generate_new_UV2_layout && do_generate_new_UV2_layout == false)
            {
        ...
        {
            Debug.LogError("Need to add objects to this Combine before calling Apply or ApplyAll");
        }
    }
    ...
    {
        if (LOG_LEVEL >= SA2LogLevel.debug) Debug.Log("UpdateGameObjects called on " + gos.Length + " objects.");
        _initialize();
        if (_mesh.vertexCount > 0 && _instance2combined_map.Count == 0)
        {
            Debug.LogWarning("There were vertices in the combined mesh but nothing in the Combine buffers. If you are trying to bake in the editor and modify at runtime, make sure 'Clear Buffers After Bake' is unchecked.");
                                    ...
                                    bool updateUV, bool updateUV1, bool updateUV2, bool updateColors, bool updateSkinningInfo)
    {
        ...
        if (renderType == SARenderType.skinnedMeshRenderer && updateSkinningInfo)
        {
        ...
        if (_doUV1 && updateUV1)
        {
            Vector2[] nuv1 = SAVersion.GetMeshUV1s(mesh, LOG_LEVEL);
    ...
    public override bool AddDeleteGameObjectsByID(GameObject[] gos, int[] deleteGOinstanceIDs, bool disableRendererInSource)
    {
        ...
        if (validationLevel > SA2ValidationLevel.none)
        {
            ...
            if (gos != null)
            {
                for (int i = 0; i < gos.Length; i++)
                {
                    ...
                    if (validationLevel >= SA2ValidationLevel.robust)
                    {
            ...
            if (deleteGOinstanceIDs != null && validationLevel >= SA2ValidationLevel.robust)
            {
        ...
        //if (_outputOption == SA2OutputOptions.bakeIntoSceneObject || 
        //   (_outputOption == SA2OutputOptions.bakeIntoPrefab && _renderType == SARenderType.skinnedMeshRenderer) ||
        //	_outputOption == SA2OutputOptions.bakeMeshAssetsInPlace){
    ...
    public override void DestroyMesh()
    {
        if (_mesh != null)
        {
            if (LOG_LEVEL >= SA2LogLevel.debug) SA2Log.LogDebug("Destroying Mesh");
            SAUtility.Destroy(_mesh);
        }
        _mesh = new Mesh();
        ClearBuffers();
    }

    public override void DestroyMeshEditor(SA2EditorMethodsInterface editorMethods)
    {
        if (_mesh != null)
        {
            if (LOG_LEVEL >= SA2LogLevel.debug) SA2Log.LogDebug("Destroying Mesh");
    ...
    public bool ValidateTargRendererAndMeshAndResultSceneObj()
    {
        ...
        {
            if (_LOG_LEVEL >= SA2LogLevel.error) Debug.LogError("Result Scene Object was not set.");
            return false;
        }
        else
        {
            if (_targetRenderer == null)
            {
                if (_LOG_LEVEL >= SA2LogLevel.error) Debug.LogError("Target Renderer was not set.");
                return false;
            }
            else
            {
                ...
                {
                    if (_LOG_LEVEL >= SA2LogLevel.error) Debug.LogError("Target Renderer game object is not a child of Result Scene Object was not set.");
                    return false;
                }
                if (_renderType == SARenderType.skinnedMeshRenderer)
                {
                    if (!(_targetRenderer is SkinnedMeshRenderer))
                    {
                        if (_LOG_LEVEL >= SA2LogLevel.error) Debug.LogError("Render Type is skinned mesh renderer but Target Renderer is not.");
                        return false;
                    }
                    if (((SkinnedMeshRenderer)_targetRenderer).sharedMesh != _mesh)
                    {
                        if (_LOG_LEVEL >= SA2LogLevel.error) Debug.LogError("Target renderer mesh is not equal to mesh.");
                        return false;
                    }
                } if (_renderType == SARenderType.meshRenderer)
                {
                    if (!(_targetRenderer is MeshRenderer))
                    {
                        if (_LOG_LEVEL >= SA2LogLevel.error) Debug.LogError("Render Type is mesh renderer but Target Renderer is not.");
                        return false;
                    }
    ...
    public static Renderer BuildSceneHierarch(SA3MeshCombinerSingle mom, GameObject root, Mesh m, bool createNewChild = false)
    {
        if (mom._LOG_LEVEL >= SA2LogLevel.trace) Debug.Log("Building Scene Hierarchy createNewChild=" + createNewChild);
        GameObject meshGO;
        MeshFilter mf = null;
        ...
        if (mom.renderType == SARenderType.skinnedMeshRenderer)
        {
            MeshRenderer r = meshGO.GetComponent<MeshRenderer>();
            if (r != null) SAUtility.Destroy(r);
            MeshFilter f = meshGO.GetComponent<MeshFilter>();
            if (f != null) SAUtility.Destroy(f);
        ...
        if (mom.lightmapOption == SA2LightmapOptions.preserve_current_lightmapping || mom.lightmapOption == SA2LightmapOptions.generate_new_UV2_layout)
        {
    ...
    public void BuildSceneMeshObject(bool createNewChild = false)
    {
        if (_resultSceneObject == null)
        {
            _resultSceneObject = new GameObject("SASMesh-" + name);
        }
