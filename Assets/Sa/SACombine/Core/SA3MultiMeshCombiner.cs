...
public class SA3MultiMeshCombiner : SA3MeshCombiner
{
    public override SA2LogLevel LOG_LEVEL
    {
    ...
    public override SA2ValidationLevel validationLevel
    {
        set
        {
    ...
    {
        public SA3MeshCombinerSingle combinedMesh;
    ...
    //		SA2Log.Log(SA2LogLevel.info,"num combiners " + meshCombiners.Count + " num children " + meshCombiners[0].combinedMesh.resultSceneObject.transform.childCount);
    //		for (int i = 0; i < meshCombiners.Count; i++){
    ...
    public override void Apply(SA3MeshCombiner.GenerateUV2Delegate uv2GenerationMethod)
    {
        for (int i = 0; i < meshCombiners.Count; i++)
        {
    ...
    public override void Apply(bool triangles,
                      ...
                      bool uv2,
                      bool bones = false,
                      SA3MeshCombiner.GenerateUV2Delegate uv2GenerationMethod = null)
    {
        for (int i = 0; i < meshCombiners.Count; i++)
        {
            if (meshCombiners[i].isDirty)
    ...
    public override bool AddDeleteGameObjectsByID(GameObject[] gos, int[] deleteGOinstanceIDs, bool disableRendererInSource = true)
    {
        ...
        if (LOG_LEVEL >= SA2LogLevel.debug) SA2Log.LogDebug("SA2MultiMeshCombiner.AddDeleteGameObjects numCombinedMeshes: " + meshCombiners.Count + " added:" + gos + " deleted:" + deleteGOinstanceIDs + " disableRendererInSource:" + disableRendererInSource + " maxVertsPerCombined:" + _maxVertsInMesh);
        return _bakeStep1(gos, deleteGOinstanceIDs, disableRendererInSource);
    }

    bool _validate(GameObject[] gos, int[] deleteGOinstanceIDs)
    {
        if (_validationLevel == SA2ValidationLevel.none) return true;
        ...
        if (gos != null)
        {
            for (int i = 0; i < gos.Length; i++)
            {
                ...
                if (_validationLevel >= SA2ValidationLevel.robust)
                {
    ...
    void _distributeAmongBakers(GameObject[] gos, int[] deleteGOinstanceIDs)
    {
        ...
        if (resultSceneObject == null) resultSceneObject = new GameObject("SAMMesh-" + name);
        ...
        for (int i = 0; i < deleteGOinstanceIDs.Length; i++)
        {
            ...
            {
                if (LOG_LEVEL >= SA2LogLevel.debug) SA2Log.LogDebug("SA2MultiMeshCombiner.Removing " + deleteGOinstanceIDs[i] + " from meshCombiner " + meshCombiners.IndexOf(c));
        ...
        for (int i = 0; i < gos.Length; i++)
        {
            ...
            for (int j = 0; j < meshCombiners.Count; j++)
            {
                ...
                {
                    cm = meshCombiners[j];
                    if (LOG_LEVEL >= SA2LogLevel.debug) SA2Log.LogDebug("SA2MultiMeshCombiner.Added " + gos[i] + " to combinedMesh " + j, LOG_LEVEL);
                    break;
                }
            }
            if (cm == null)
            {
                ...
                if (LOG_LEVEL >= SA2LogLevel.debug) SA2Log.LogDebug("SA2MultiMeshCombiner.Created new combinedMesh");
            }
            cm.gosToAdd.Add(go);
    ...
    bool _bakeStep1(GameObject[] gos, int[] deleteGOinstanceIDs, bool disableRendererInSource)
    {
        ...
        for (int i = 0; i < meshCombiners.Count; i++)
        {
            ...
            if (cm.combinedMesh.targetRenderer == null)
            {
                ...
                if (_LOG_LEVEL >= SA2LogLevel.debug) SA2Log.LogDebug("BuildSO combiner {0} goID {1} targetRenID {2} meshID {3}", i, cm.combinedMesh.targetRenderer.gameObject.GetInstanceID(), cm.combinedMesh.targetRenderer.GetInstanceID(), cm.combinedMesh.GetMesh().GetInstanceID());
            ...
            if (cm.gosToAdd.Count > 0 || cm.gosToDelete.Count > 0)
            {
                ...
                if (_LOG_LEVEL >= SA2LogLevel.debug) SA2Log.LogDebug("Baked combiner {0} obsAdded {1} objsRemoved {2} goID {3} targetRenID {4} meshID {5}", i, cm.gosToAdd.Count, cm.gosToDelete.Count, cm.combinedMesh.targetRenderer.gameObject.GetInstanceID(), cm.combinedMesh.targetRenderer.GetInstanceID(), cm.combinedMesh.GetMesh().GetInstanceID());
            }
