...
namespace SA.Core
{
    public class SAVersionConcrete:SAVersionInterface
    {
...
#else
            if (LOG_LEVEL >= SA2LogLevel.warn) SA2Log.LogDebug("UV1 does not exist in Unity 5+");
            uv = m.uv;
#endif
            if (uv.Length == 0)
            {
                if (LOG_LEVEL >= SA2LogLevel.debug) SA2Log.LogDebug("Mesh " + m + " has no uv1s. Generating");
                if (LOG_LEVEL >= SA2LogLevel.warn) Debug.LogWarning("Mesh " + m + " didn't have uv1s. Generating uv1s.");
